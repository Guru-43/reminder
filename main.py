...
def main() -> None:
    ...
    calendar = sheet.fetch('calendar')            # College calendar 
    ...
    _range = f'periods!A{day_order}:F{day_order}' # periods range for the day order
    periods = sheet.fetch(_range)[0]              # classes for the day order
    _range = f'staffs!A{day_order}:F{day_order}'  # staffs range for the day order 
    ...
    subject = f'Today College Schedule {today}'
